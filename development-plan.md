...
| 14 патологий (CheXpert) | Chest X-Ray | Мультиметочная классификация (14 классов) |
...
│   └── tool.py
├── chexpert/            # Инструмент 2: CheXpert 14-class Multi-Label Classifier
│   └── tool.py
├── brain_tumor/         # Инструмент 3 (будущий): BraTS Segmentation
│   └── tool.py
└── dermoscopy/          # Инструмент 4 (будущий): ISIC Classification
    └── tool.py
```
...
**14 патологий (классов):**
No Finding, Enlarged Cardiomediastinum, Cardiomegaly, Lung Opacity, Lung Lesion, Edema, Consolidation, Pneumonia, Atelectasis, Pneumothorax, Pleural Effusion, Pleural Other, Fracture, Support Devices.

**Особенность датасета — неопределённые метки (uncertainty labels):** часть меток размечена как «U» (неопределённо). Стандартные стратегии: U-Zeros (заменять 0), U-Ones (заменять 1), U-Ignore (маскировать в лоссе), U-SelfTraining (pseudo-labeling). Опорная статья CheXpert рекомендует U-Ones для части классов.

**Архитектура и пайплайн:**
- Backbone: DenseNet-121 (тот же, что и в RSNA) — можно инициализировать от NIH pretrain-чекпоинта
- Выходной слой: 14 нейронов + sigmoid (независимые бинарные классификаторы)
- Лосс: BCEWithLogitsLoss с маскированием U-меток (U-Ignore стратегия)
- Входной размер: 320×320 или 384×384, фронтальная проекция (PA/AP)
- AMP + mixed precision, как в RSNA

**Скрипты (добавить в `backend/ml/training/`):**

| Скрипт | Описание |
|---|---|
| `chexpert_dataset.py` | `CheXpertDataset` — загрузка CSV-разметки, обработка U-меток, MONAI transforms |
| `train_chexpert.py` | Обучение DenseNet-121 с BCEWithLogitsLoss + U-Ignore маской |
| `eval_chexpert.py` | Per-class AUC ROC + mean AUC; confusion matrix для каждого класса |

**Конфиг:** `backend/ml/configs/chexpert_densenet121.yaml`

**Метрика качества:** mean AUC по всем 14 классам ≥ 0.85; приоритетные 5 классов из оригинальной статьи: Atelectasis, Cardiomegaly, Consolidation, Edema, Pleural Effusion — target mean AUC ≥ 0.88.

**Реестр инструментов:**
- `backend/app/modules/ml_tools/chexpert/tool.py` — `CheXpertTool(MLTool)` с `TOOL_ID = "chexpert_14"`, возвращает `ClassificationResult` со списком из 14 пар `(label, probability)`
- Grad-CAM применяется per-class: для каждой патологии с вероятностью > порога генерируется отдельная тепловая карта

**Данные:** датасет скачивается с [physionet.org/content/mimic-cxr](https://physionet.org/content/mimic-cxr) или Stanford CheXpert портала (требует регистрации). Локальный путь: `backend/ml/data/chexpert/` (gitignored). Чекпоинты — MinIO `mida-models/weights/chexpert_*/`.
...
- Бинарная классификация (RSNA Pneumonia): AUC ROC ≥ 0.90, Sensitivity ≥ 0.85
- Мультиметочная классификация (CheXpert 14): mean AUC ≥ 0.85 по всем 14 классам; приоритетные 5 (Atelectasis, Cardiomegaly, Consolidation, Edema, Pleural Effusion) — mean AUC ≥ 0.88

Для CheXpert отчитываться per-class AUC отдельно — «среднее» скрывает слабые классы (Fracture, Lung Lesion малочисленны). Оценка включает матрицу ROC-кривых по всем 14 классам.
